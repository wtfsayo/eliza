...
elizaos plugins list [--type <type>]
...
- `elizaos plugins list` (aliases: `ls`, `l`): Lists available plugins from the registry. Options:
  - `-t, --type <type>`: Filter by plugin type (e.g., `adapter`, `client`, `plugin`).
