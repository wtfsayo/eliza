...
// For creating a memory with similar content but different embedding
export const createSimilarMemory = (
  original: TestMemory,
  similarity: number = 0.9
): TestMemory & { embedding: number[] } => {
  const originalWithEmbedding = embeddingTestMemoriesWithEmbedding.find(
    (m) => m.id === original.id
  );
  if (!originalWithEmbedding || !originalWithEmbedding.embedding) {
    throw new Error("Original memory doesn't have embedding");
  }

  // Create a similar embedding vector
  const originalVector = originalWithEmbedding.embedding;
  const similarVector = originalVector.map((v) => {
    // Add some noise while maintaining similarity
    const noise = (Math.random() * 2 - 1) * (1 - similarity) * 0.1;
    return v * similarity + noise;
  });

  return {
    id: fixedUuid(Math.floor(Math.random() * 1000 + 100)),
    entityId: original.entityId,
    agentId: original.agentId,
    roomId: original.roomId,
    createdAt: Date.now(),
    content: {
      text: `This is similar to: ${(original.content as any).text}`,
      type: 'text',
    },
    unique: true,
    type: original.type,
