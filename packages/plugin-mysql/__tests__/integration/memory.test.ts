...
describe('Memory Integration Tests', () => {
  ...
  describe('Memory CRUD Operations', () => {
    ...
    it('should perform partial updates without affecting other fields', async () => {
      ...
      // Compare embeddings after rounding to handle potential precision differences
      const roundArray = (arr: number[] | undefined, precision: number): number[] | undefined => {
        if (!arr) return undefined;
        return arr.map((num) => parseFloat(num.toFixed(precision)));
      };
      const roundedRetrievedEmbedding = roundArray(afterContentUpdate?.embedding, 6);
      const roundedOriginalEmbedding = roundArray(memory.embedding, 6);
      expect(roundedRetrievedEmbedding).toEqual(roundedOriginalEmbedding);
      ...
      // Also verify embedding remained unchanged after metadata update
      const roundedMetadataEmbedding = roundArray(afterMetadataUpdate?.embedding, 6);
      expect(roundedMetadataEmbedding).toEqual(roundedOriginalEmbedding);
    });
